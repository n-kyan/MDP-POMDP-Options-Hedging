...
| $\tau_0$               | Initial time to expiration             | 30 trading days                                              |
...
| GLF-T + WW | $4.004 | $2.366 | 1.692 | $0.377 | 36.8% | 0.090 | $1.511 |
| GLF-T + Naive | $3.677 | $1.986 | 1.852 | $0.377 | 69.9% | 0.073 | $1.896 |
| Naive + WW | $4.405 | $4.320 | 1.020 | $0.101 | 48.3% | 0.171 | $4.676 |
| Naive + Naive | $3.852 | $3.713 | 1.037 | $0.101 | 79.9% | 0.146 | $5.237 |

**Spread policy.** The GLF-T spread formula, which scales the half-spread with portfolio gamma, time to expiry, and variance, produces spreads averaging $\$0.38$ — roughly 3.8x wider than the fixed $\$0.10$ spread used by the Naive policies. This is not aggressive quoting: the GLF-T spread reflects the true inventory-risk cost of a fill. The much narrower fixed spread generates more fills and higher gross P&L on average, but at the cost of taking on more unhedged delta exposure, which manifests as larger $|\hat\Delta_P|$ and dramatically higher P&L variance.

**Hedging policy.** The WW no-trade band allows $\hat\Delta_P$ to drift within a tolerance before hedging, resulting in a 37% hedge frequency and lower transaction costs. The Naive hedge (always target $\Delta^{\text{target}} = 0$) trades approximately twice as often, incurring higher costs, but keeps $|\hat\Delta_P|$ tighter on average. 

**The GLF-T + Naive paradox.** The highest Sharpe ratio across both environments is achieved by GLF-T + Naive (1.886 constant vol, 1.852 Hardy), which combines the analytically derived spread with the simplest possible hedging rule. GLF-T + WW, despite incorporating a theoretically superior hedging policy, has lower Sharpe in both environments. The explanation lies in variance: aggressive full-hedging keeps the P&L distribution tighter (std $1.93 vs $2.24 on constant vol), and the Sharpe ratio rewards this even though mean P&L is lower. In this parameterization, the WW band width is wide enough to allow meaningful delta accumulation between hedges, which adds variance without sufficient compensating spread income. This is a parameterization-specific result — with higher $\varphi$ (stronger risk aversion) or wider spreads, the WW band advantage would likely reassert itself.

**Regime-switching vs constant vol.** Mean P&L is similar across both environments for the GLF-T policies ($3.97 vs $4.00, $3.64 vs $3.68), indicating that the oracle volatility calibration correctly accounts for the regime distribution. The Naive spread policies show a more pronounced decline on Hardy ($4.62 → $4.40, $4.17 → $3.85) because their fixed spread is not adjusted to compensate for the additional volatility risk in the high-vol regime.
...
|---|---|---|---|
| GLF-T + Naive (best benchmark) | $3.677 | $1.986 | 1.852 |
| GLF-T + WW | $4.004 | $2.366 | 1.692 |
| **MCTS-DPW (oracle MDP)** | **$2.697** | **$2.805** | **0.962** |

The MCTS-DPW solver, despite receiving the same oracle information as the analytical benchmarks, underperforms all four of them on both mean P&L and Sharpe. This result warrants careful interpretation.

**What MCTS-DPW was given.** The MCTS agent observes the true regime index at every step, has full access to the transition matrix, and prices options using the identical belief-weighted Black-Scholes formula as the benchmarks. It solves a fully observable MDP. The only disadvantage relative to the analytical benchmarks is that it must discover good spread and hedge actions from simulation rather than deriving them from closed-form theory.

**Why it underperforms.** The fundamental constraint is the computation budget. Each step, MCTS-DPW runs 200 iterations of Double Progressive Widening from a fresh tree root, planning 20 steps ahead before handing off to a GLF-T+WW rollout. With DPW parameters $k=2$, $\alpha=0.5$, 200 iterations produces approximately $k \cdot 200^\alpha \approx 28$ unique actions at the root, each sampled roughly 7 times. A rollout that runs for the remaining 130 steps of the episode introduces high variance in the value estimate. Seven samples drawn from a distribution with episode-level variance of $\approx \$3$ (std of rewards) cannot reliably rank actions whose expected value difference is on the order of cents per step. The Q-value estimates are too noisy to consistently select better-than-random actions at the root.

In short: 200 MCTS queries per step is insufficient to empirically rediscover what GLFT derives analytically in closed form. The information is available — the state is fully observed, the transition model is correct — but the statistical sample size is too small to exploit it.

**A note on what this finding implies.** This result does not indict MCTS as a method; it quantifies the computational cost of discovering structure that the analytical benchmarks encode for free. If the action space were not well-characterized by domain theory, or if the problem structure were novel enough that no closed-form solution existed, MCTS with a larger query budget would be the correct tool. The takeaway for this problem specifically is that the analytical benchmarks are hard to beat precisely because they distill decades of microstructure theory into a handful of formulas. An MCTS agent with $10\times$ the query budget might approach, but is unlikely to surpass, the Sharpe of GLF-T + Naive without incorporating domain structure into the action prior.

## 6. Conclusion

This paper formulated options market making as a POMDP and implemented a complete solution: a particle filter belief over continuous volatility, a Black-Scholes-based observation and pricing model, four analytical benchmark policies, and an MCTS-DPW oracle MDP solver. All components were evaluated on a Hardy (2001) regime-switching simulator with parameters calibrated to historical S&P 500 returns.

The central empirical finding is that **analytical structure dominates general-purpose search at realistic compute budgets**. The best analytical policy (GLF-T + Naive) achieves a Sharpe of 1.85 on the Hardy environment, while the MCTS oracle MDP — despite having identical information — achieves 0.96. The gap is not an information disadvantage; it is the cost of having to rediscover empirically what the GLFT derivation provides for free. This has a direct practical implication: in well-studied domains where closed-form solutions exist for sub-problems, hybrid architectures — domain theory for the base policy, learned adjustments for the residual — will outperform pure search solvers at any realistic computational budget.

A secondary finding, less immediately obvious, is that **the theoretically superior hedging policy (WW no-trade band) does not dominate the naive full-hedge in this parameterization**. GLF-T + Naive beats GLF-T + WW on Sharpe in both the constant-vol and Hardy environments because more frequent hedging reduces P&L variance enough to offset the higher transaction costs. This is a parameterization-specific result sensitive to the spread width, risk aversion $\varphi$, and hedge cost $c$, but it illustrates a general point: theoretical optimality of a component policy does not guarantee optimality of the composite system when the performance metric is Sharpe rather than expected P&L.

The POMDP framing, while not fully realized in the MDP-MCTS implementation, provides the correct conceptual architecture. The particle filter correctly abstracts the volatility uncertainty as a continuous belief, and the oracle MDP experiment cleanly separates the cost of partial observability (which the POMDP agent would bear) from the cost of policy approximation (which both the MDP agent and the benchmarks bear equally). The next step is to run the same MCTS planner on the particle filter belief state rather than the oracle state, and to measure the performance degradation from partial observability alone.
